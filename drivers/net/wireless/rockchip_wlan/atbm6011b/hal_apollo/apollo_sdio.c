...
static int atbm_sdioirq_en(struct sbus_priv *self,bool en)
{
	int ret = 0;
	u8 cccr;
	//atbm_printk_err("entring %s+++\n",__func__);
	struct atbm_common *hw_priv = self->core;

	if(hw_priv->init_done == 0)
		return 0;
	printk_once("[atbm_log]:%s\n",__func__);
...
{
	int ret = 0;
//	atbm_printk_err("%s+++(%x)count:%d\n",__func__,addr,count);
	atbm_sdioirq_en(self,false);
	ret = sdio_memcpy_fromio(self->func, dst, addr, count);
//	atbm_printk_err("%s---(%x)\n",__func__,addr);
	atbm_sdioirq_en(self,true);
	return ret;
}
				   ...
				   const void *src, int count)
{
	...
	case sizeof(u16):
		//msleep(50);
		//atbm_printk_err("%s u16+++(%x)\n",__func__,*(u16 *)src);
		sdio_writew(self->func, *(u16 *)src, addr, &ret);
		//atbm_printk_err("%s---(%x)\n",__func__,sdio_readw(self->func,addr, &ret));
		break;
	case sizeof(u32):
		//msleep(50);
		//atbm_printk_err("%s u32+++(%x)(%x)\n",__func__,addr,*(u32 *)src);
		sdio_writel(self->func, *(u32 *)src, addr, &ret);
		//atbm_printk_err("%s---(%x)(%x)\n",__func__,addr,sdio_readl(self->func, addr, &ret));
		break;
	default:
			//atbm_printk_err("%s u8 +++\n",__func__);
		WARN_ON(count == 8);

				    ...
				    struct sk_buff *txskb)
{
 ...
 out_check_combine:
	list_for_each_entry(tmp, &local->roc_list, list) {
		...
		if (local->ops->remain_on_channel) {
		...
		} else if (atbm_del_timer_sync((struct atbm_timer_list *)&tmp->work.timer)) {
			...
			if (time_after(new_end, tmp->work.timer.expires))
				atbm_mod_timer((struct atbm_timer_list *)&tmp->work.timer, new_end);
			else
				atbm_add_timer((struct atbm_timer_list *)&tmp->work.timer);

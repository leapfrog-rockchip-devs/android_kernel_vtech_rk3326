...
#endif
#endif	
		{SIOCIWFIRSTPRIV + 0x1a, IW_PRIV_TYPE_CHAR | 10, 0, "get_last_mac"},
		{SIOCIWFIRSTPRIV + 0x1b, IW_PRIV_TYPE_CHAR | 10, 0, "get_first_mac"},
};
#ifdef CONFIG_ATBM_PRIV_HELP
static int atbm_ioctl_command_help(struct net_device * dev,struct iw_request_info * ifno,union iwreq_data * wrqu,char * ext)
{
	int ret = 0;
		...
		atbm_printk_wext("get_last_mac		NO Parameter = get last mac address\n");
		atbm_printk_wext("get_first_mac		NO Parameter = get first mac address\n");
	return ret;
}
#endif
/**************************************************************************
...
static int atbm_ioctl_get_Last_mac(struct net_device *dev, struct iw_request_info *info, union iwreq_data *wrqu, char *extra)
{
	...
	struct iw_point iwp;
	iwp_compat = (struct compat_iw_point *) &wrqu->data;
	iwp.length = iwp_compat->length;
	printk("iwp.length:%d\n",iwp.length);
	printk("wrqu->data.length:%d\n",wrqu->data.length);	
	if(iwp.length > 1)
#else
	if(wrqu->data.length > 1)
#endif
...
#ifdef CONFIG_ARM64
	atbm_printk_wext("atbm_ioctl_get_rssi() size %d , \n",wdata->data.length);
#else
...
#ifdef CONFIG_ARM64	
	atbm_printk_wext("atbm: ch %u, freq %zu\n", req.channel_num, req.freq);
#else
...
#ifdef CONFIG_ARM64	
	printk("atbm_ioctl_send_singleTone:%zu,%d\n",strlen(extra), wrqu->data.length);
#else
...
#endif
#endif	
	[26] = (iw_handler)atbm_ioctl_get_Last_mac,
	[27] = (iw_handler)atbm_ioctl_get_First_mac,

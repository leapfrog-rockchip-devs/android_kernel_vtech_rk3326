...
int atbm_register_bh(struct atbm_common *hw_priv)
{
	...
	if (IS_ERR(hw_priv->bh_thread)) {
		...
		sched_setscheduler(hw_priv->bh_thread,SCHED_FIFO, &param);
...
restart:
	...
	hw_priv->sbus_ops->lock(hw_priv->sbus_priv);
	atbm_sdio_process_read_data(hw_priv,atbm_bh_read_ctrl_reg_unlock,atbm_data_read_unlock,atbm_rx_directly,ATBM_RX_DERICTLY_DATA_FRAME);	
	hw_priv->sbus_ops->unlock(hw_priv->sbus_priv);
}

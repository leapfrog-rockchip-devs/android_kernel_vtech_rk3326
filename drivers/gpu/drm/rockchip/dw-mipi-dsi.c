...
int of_find_panel_or_bridge(const struct device_node *np,struct drm_panel **panel,   
				struct drm_bridge **bridge)                                                   
{                                                                                    
  ...
  int ret = -EPROBE_DEFER;                                                           
    ...
    for_each_child_of_node(node, port) {                                             
        ...
        of_property_read_u32(port, "reg", &port_id);  
         printk("frank debug port_id=%d",port_id);                               
        if (port_id > 0){                                                            
        	ret = drm_of_find_panel_or_bridge(np, port_id, -1, panel, bridge);          
          if(ret != -ENODEV)                                                         
        	    break;                                                                  
        }                                                                            
...
*/
	ret  = of_find_panel_or_bridge(dev->of_node, &dsi->panel, &dsi->bridge);
	if (ret) {
		DRM_DEV_ERROR(dev, "Failed to find panel or bridge: %d\n", ret);
		return ret;
	}

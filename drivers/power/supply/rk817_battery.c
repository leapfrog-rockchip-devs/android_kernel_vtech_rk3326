...
int get_charge_current =300;
extern int charge_online;
//#define NTCdebug
...
enum rk817_battery_fields {
	...
	VOL_ADC_K3, VOL_ADC_K2, VOL_ADC_K1, VOL_ADC_K0,CHRG_CUR_SEL,
	USB_ILIM_SEL, CHRG_EN,BAT_EXS, CHG_STS, BAT_OVP_STS, CHRG_IN_CLAMP,
...
static int rk817_bat_get_nts_res(struct rk817_battery_device *battery)
{
	...
	res = bat_ts_vol * 1000 / battery->pdata->ntc_factor;
	if (battery->temperature<200) {
		rk817_bat_field_write(battery, VOL_ADC_TSCUR_SEL, FLOW_OUT_40uA);
        	battery->pdata->ntc_factor = 21;
	} else if(battery->temperature>300){
		rk817_bat_field_write(battery, VOL_ADC_TSCUR_SEL, FLOW_OUT_80uA);
          battery->pdata->ntc_factor = 43;
	}
		#ifdef NTCdebug
		printk("++++++++++++ bat_ts_vol: %d ntc_factor=%d,battery->temperature=%d,res=%d\n\n\n", bat_ts_vol,battery->pdata->ntc_factor,battery->temperature,res);
	#endif
	return res;
}
...
static void rk817_bat_update_temperature(struct rk817_battery_device *battery)
{
	...
	ntc_size = battery->pdata->ntc_size;
	  get_charge_current=rk817_bat_get_avg_current(battery);
//	battery->temperature = VIRTUAL_TEMPERATURE;
	...
	//	printk("+++++++++res++ %d\n", res);
		...
		if (res < ntc_table[ntc_size - 1]) {
			//printk("bat ntc upper max degree: R=%d\n", res);
				battery->temperature = 500;
		} else if (res > ntc_table[0]) {
		//	printk("bat ntc lower min degree: R=%d\n", res);
				battery->temperature = (0  + battery->pdata->ntc_degree_from) * 10;
		} else {
			for (i = 0; i < ntc_size; i++) {
				if (res >= ntc_table[i])
					break;
			}
			battery->temperature = (i /* * 2*/ + battery->pdata->ntc_degree_from) * 10;
			//printk("+++++++++res++ %d,i=%d,battery->temperature=%d\n", res,i,battery->temperature);
		}
	}
	//printk("+++++++++res++ battery->temperature=%d\n",battery->temperature);
	if ((battery->temperature <= 0) || (battery->temperature > 450)) {
		#ifdef NTCdebug
			printk("frank debug test charge \n\n\n\n");
		#endif	
			if(is_bat_charge_en(battery))
			{
					//#ifdef NTCdebug
				  printk("frank debug disable charge \n\n\n\n");
				 // #endif
	...
	else 
	{
			...
			{
					//#ifdef NTCdebug
					printk("frank debug enable charge \n\n\n\n");
					//#endif
					rk817_bat_enable_charge(battery);
					#ifdef NTCdebug
					printk("enable charge battery->usb_in=%d,battery->ac_in=%d\n",battery->usb_in ,battery->ac_in );    
					#endif
				
			}
			if(battery->usb_in==1)
				{
							rk817_bat_field_write(battery, USB_ILIM_SEL,INPUT_CUR_450MA);
				}
				else
				{
						
					rk817_bat_field_write(battery, USB_ILIM_SEL,INPUT_CUR_1500MA);
					if(battery->temperature <150)
					{
						//printk("frank debug CHRG_CUR_500MA \n\n\n\n");
	...
	#ifdef NTCdebug
//	printk("+++++++++++last_temp%d battery->temperature=%d\n", last_temp,battery->temperature);
	#endif
	if(last_temp != battery->temperature)

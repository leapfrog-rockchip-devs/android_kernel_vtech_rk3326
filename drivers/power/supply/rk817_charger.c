...
static int dbg_enable=0;
 extern void led_setcharging_gpiovalue(int state);
 extern void led_setfull_gpiovalue(int state);
int charge_online=0;
extern int get_charge_current;
					 ...
					 union power_supply_propval *val)
{
	...
	{
		charge_online=1;
	//	get_current=rk817_bat_get_pwron_current(charge);
		//if((rk817_charge_get_dsoc(charge) == 100)&&(current<100))
		if(rk817_charge_get_dsoc(charge) <100)
		{
				led_setcharging_gpiovalue(0);
				led_setfull_gpiovalue(1);
		}
		else
		{
			if(get_charge_current<200)
			{
				charge->prop_status = POWER_SUPPLY_STATUS_FULL;
				led_setcharging_gpiovalue(1);
				led_setfull_gpiovalue(0);
			}
		}
	}
	else
	{
		charge_online=0;
		led_setfull_gpiovalue(0);//led off
		led_setcharging_gpiovalue(0);//led off
	}
...
{
	rk817_charge_field_write(charge, SYS_CAN_SD, ENABLE);
}

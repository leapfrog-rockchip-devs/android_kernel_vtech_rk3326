...
#else
u8 _gGestureWakeupPacket[GESTURE_WAKEUP_PACKET_LENGTH] = {0}; // packet length = GESTURE_WAKEUP_PACKET_LENGTH
...
u8 *_gPlatformFwVersion = NULL; // internal use firmware version
...
s32 g_MpTestMode = 2;
u8 g_SystemSuspend = DISABLE_SUSPEND;;
//------------------------------------------------------------------------------//
...
u16 DEMO_MODE_PACKET_LENGTH = 0; // If project use MSG28xx, set MUTUAL_DEMO_MODE_PACKET_LENGTH as default.
u16 DEBUG_MODE_PACKET_LENGTH = 0; // If project use MSG28xx, set MUTUAL_DEBUG_MODE_PACKET_LENGTH as default.
u16 MAX_TOUCH_NUM = 0; // If project use MSG28xx, set MUTUAL_MAX_TOUCH_NUM as default.
...
u8 g_DemoModePacket[MUTUAL_DEMO_MODE_PACKET_LENGTH] = {0}; //for MSG28xx/MSG58xxA : DEMO_MODE_PACKET_LENGTH = MUTUAL_DEMO_MODE_PACKET_LENGTH, for ILI21xx : DEMO_MODE_PACKET_LENGTH = ILI21XX_DEMO_MODE_PACKET_LENGTH

MutualFirmwareInfo_t g_MutualFirmwareInfo;

u8 g_DebugModePacket[MUTUAL_DEBUG_MODE_PACKET_LENGTH] = {0}; //for MSG28xx/MSG58xxA/ILI21xx : DEBUG_MODE_PACKET_LENGTH = MUTUAL_DEBUG_MODE_PACKET_LENGTH
u8 g_LogModePacket[MUTUAL_DEBUG_MODE_PACKET_LENGTH] = {0}; //for MSG28xx/MSG58xxA/ILI21xx : DEBUG_MODE_PACKET_LENGTH = MUTUAL_DEBUG_MODE_PACKET_LENGTH
u16 g_FirmwareMode;
...
{    
    ...
    if (pStr != NULL)
    {
        ...
        {
            g_GestureWakeupMode[0] = g_GestureWakeupMode[0] & (~GESTURE_WAKEUP_MODE_CLICK_FLAG);
        }
        if ((nWakeupMode[0] & GESTURE_WAKEUP_MODE_CLICK_FLAG) == GESTURE_WAKEUP_MODE_CLICK_FLAG)
        {
            g_GestureWakeupMode[0] = g_GestureWakeupMode[0] | GESTURE_WAKEUP_MODE_CLICK_FLAG;
        }
        else
...
//    _gGpioRst = of_get_named_gpio_flags(pDeviceNode, "chipone,reset-gpio", 0, &nFlag);
   ...
   //	  _gGpioRst=of_get_named_gpio(pDeviceNode,"touch,reset-gpio", 0);
//    _gGpioRst = of_get_named_gpio_flags(pDeviceNode, "mstar,rst-gpio", 0, &nFlag); // generally used for MSG28XX/MSG58XXA
    ...
    if (_gGpioRst < 0)
    {
		...
		//	_gGpioIrq = of_get_named_gpio(pDeviceNode, "touch,irq-gpio", 0);
...
#ifdef CONFIG_ENABLE_GESTURE_WAKEUP
    if(g_SystemSuspend) {
        DBG(&g_I2cClient->dev, "SYSTEM IN SUSPEND, g_SystemSuspend = %d\n", g_SystemSuspend);
        input_report_key(g_InputDevice, KEY_POWER, 1);
        input_sync(g_InputDevice);
        input_report_key(g_InputDevice, KEY_POWER, 0);
        input_sync(g_InputDevice);        
    ...
    {
        {
            for (i = 0; i < MAX_TOUCH_NUM; i ++)
            {
                ...
                pInfo->tPoint[pInfo->nCount].nP = 10;//pPacket[4*(i+1)];
...
#endif //CONFIG_PLATFORM_USE_ANDROID_SDK_6_UPWARD
{
...
#ifdef CONFIG_ENABLE_GESTURE_WAKEUP
    #ifdef CONFIG_ENABLE_CLICK_FOR_DRIVER
    DrvSetIdleMode();
    enable_irq_wake(_gIrq);
    return;
    #else
    ...
    #endif //CONFIG_ENABLE_CLICK_FOR_DRIVER
#endif //CONFIG_ENABLE_GESTURE_WAKEUP
    DrvFingerTouchReleased(0, 0, 0); // Send touch end for clearing point touch
    input_sync(g_InputDevice);
...
s32 /*__devinit*/ MsDrvInterfaceTouchDeviceProbe(struct i2c_client *pClient, const struct i2c_device_id *pDeviceId)
{
	...
	u16 nMajor = 0, nMinor = 0;
    DBG(&g_I2cClient->dev, "*** %s() ***\n", __func__);
  	printk("frank debug ILI2117A touc\n\n");

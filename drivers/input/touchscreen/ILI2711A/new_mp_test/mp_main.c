...
extern void DebugShowArray2(void *pBuf, u16 nLen, int nDataType, int nCarry, int nChangeLine, char *name);
static void mp_calc_golden_range(int *goldenTbl, u16 weight, u16 golden_up, int *maxTbl, int *minTbl, int length)
{
    ...
    DebugShowArray2(goldenTbl, allnode_number, -32, 10, 16, "Golden Range");	
	if(golden_Cbg == 0)
	{
		shift_value = 0;
	}
    ...
    for (i = 0; i < length; i++) {
		...
		if(notch_flag && mp_test_data->Open_Swt != 0)
		{
    ...
    DebugShowArray2(maxTbl, allnode_number, -32, 10, 16, "Golden Max");
    DebugShowArray2(minTbl, allnode_number, -32, 10, 16, "Golden Min");
}
...
static int mp_load_ini(char * pFilePath)
{
	...
	ms_get_ini_data("RULES","BG_Sum_Range",str);
	mp_test_data->Unifor_BG_Sum_Range = ms_atoi(str);
	ms_get_ini_data("RULES","BG_Sum_Golden",str);
	mp_test_data->Unifor_BG_Golden = ms_atoi(str);
    ...
    mp_info("BG_Sum_Range:      [%d]\n", mp_test_data->Unifor_BG_Sum_Range);
    mp_info("BG_Sum_Golden:      [%d]\n", mp_test_data->Unifor_BG_Golden);
...
static int mp_main_init_var(void)
{
	...
	if(ERR_ALLOC_MEM(mp_test_result->pdeltac_buffer))
	{

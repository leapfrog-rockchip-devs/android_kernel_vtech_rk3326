...
read_data:
    for (row = 0; row < cts_dev->fwdata.rows; row++) {
        ret = cts_fw_reg_readsb(cts_dev, addr+is_diff*8*(row+1), buf, cts_dev->fwdata.cols * 2);
        if (ret) {
...
int cts_suspend_device(struct cts_device *cts_dev)
{
    ...
    if (cts_dev->rtdata.suspended) {
        printk("Suspend device while already suspended");
        return 0;
    }
    ...
    if (ret){    
        printk("Suspend device failed %d", ret);
...
int cts_resume_device(struct cts_device *cts_dev)
{
    ...
    if (!cts_plat_is_i2c_online(cts_dev->pdata, CTS_NORMAL_MODE_I2CADDR)) {
        ...
        if (firmware) {
            ...
            if (ret) {
                printk("Update default firmware failed %d", ret);
                goto err_set_program_mode;
            }
        } else {
            printk("Request default firmware failed %d, "

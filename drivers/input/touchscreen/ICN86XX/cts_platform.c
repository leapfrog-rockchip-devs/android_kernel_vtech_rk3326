...
static void cts_touch_event_timeout_timer(struct timer_list *t)
{
	struct cts_platform_data *pdata = from_timer(pdata, t, touch_event_timeout_timer);
//  printk("Touch event timeout");
...
#ifdef CFG_CTS_FORCE_UP
	timer_setup(&pdata->touch_event_timeout_timer, cts_touch_event_timeout_timer, 0);
#endif
...
int cts_plat_request_resource(struct cts_platform_data *pdata, struct i2c_client *i2c_client)
{
	#if 0
    struct device *dev = &i2c_client->dev;
	  enum of_gpio_flags rst_flags;
	  unsigned long irq_flags;
	pdata->int_gpio = of_get_named_gpio_flags(dev->of_node, "touch,irq-gpio", 0,(enum of_gpio_flags *)&irq_flags);
				
#ifdef CFG_CTS_HAS_RESET_PIN

	pdata->rst_gpio = of_get_named_gpio_flags(dev->of_node, "touch,reset-gpio", 0,(enum of_gpio_flags *)&rst_flags);
...
#endif
    u16 PosX[CFG_CTS_MAX_TOUCH_NUM] = {0};	
    u16 PosY[CFG_CTS_MAX_TOUCH_NUM] = {0};	
...
//	cts_dbg("Process touch %d msgs", num);	
    ...
    for (i = 0; i < num; i++) {
        ...
        PosX[i] = le16_to_cpu(msgs[i].x);
        PosY[i] = le16_to_cpu(msgs[i].y);

#ifdef CFG_CTS_SWAP_XY
        swap(PosX[i],PosY[i]);
#endif /* CFG_CTS_SWAP_XY */
#ifdef CFG_CTS_WRAP_X
       PosX[i] = wrap(pdata->res_y,PosX[i]);
#endif /* CFG_CTS_WRAP_X */
#ifdef CFG_CTS_WRAP_Y
      //  PosY[i] = wrap(pdata->res_y,PosY[i]);
#endif /* CFG_CTS_WRAP_Y */

		//printk("cts debug x=%d,y=%d,pdata->res_x=%d\n",PosX[i],PosY[i],pdata->res_x);
       ...
       //     i, msgs[i].id, msgs[i].event, x, y, msgs[i].pressure);
        ...
        switch (msgs[i].event) {
            ...
            case CTS_DEVICE_TOUCH_EVENT_STAY:
                ...
                //input_report_abs(input_dev, ABS_MT_POSITION_X, PosX[i]);
                //input_report_abs(input_dev, ABS_MT_POSITION_Y, PosY[i]);

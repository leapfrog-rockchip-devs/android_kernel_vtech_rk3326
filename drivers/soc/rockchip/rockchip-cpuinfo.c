...
extern const char *get_mmc_cid(void);
static int rockchip_cpuinfo_probe(struct platform_device *pdev)
{
	...
	if (system_serial_low == 0 && system_serial_high == 0) {
		const char* mmc_cid = get_mmc_cid();
		unsigned int cid_len = strlen(mmc_cid);
		
		if (cid_len <= 16) { //cpuinfo 16bit
			dev_info(dev, "mmc_cid is invalid data.\n");
			return 0;
		}
		...
		for (i = 0; i < 16; i++) {
			...
			unsigned char c_temp;
		        temp[0] = mmc_cid[cid_len-i-1];
			if (kstrtou8(temp, 16, &c_temp)) {
				dev_info(dev, "char data to u8 data fail\n");
				return 0;
			}
			buf[i] = c_temp;
		}
